...
## Load data---------------------
input <- file.path(data_in, "/03_imputeFRsep_data_log2.csv")

data <- read_csv(input)
View(data)
## output data------------------
plot_output<- file.path(data_in, "/Plots/03_1_QC_impute.pdf")
interactive_plot_output<- file.path(data_in, "/Plots/Interactive/03_1_QC_interactive_impute")

# add normalised intensity not log2 transformed
data$imputed_intensity <- 2^(data$imputed_intensity_log2)
...
for (df in Cell_list) {
  ...
  p1_protein_IDs <- qc_ids(
    ...
    sample = sample,
    grouping = ptm_collapse_key,
    intensity = imputed_intensity_log2,
    condition = r_condition,
    title = "Protein identifications per sample",
    plot = TRUE
  )
