...
# Step 1: Summarise missingness per (ptm_collapse_key, sample) pair
missing_collected <- missing_input %>%
  group_by(ptm_collapse_key, sample) %>%
  summarise(
...
#   distinct(ptm_collapse_key, sample, imputed_intensity_log2, .keep_all = TRUE) 
# View(data_impute_protti_condensed)

# join imputed data to the normalised data
data <- data_norm_input %>%
  left_join(data_impute_input %>% dplyr::select(ptm_collapse_key, sample, imputed_intensity_log2),
            by = c("ptm_collapse_key", "sample")) %>%
  left_join(missing_collected %>% dplyr::select(ptm_collapse_key, sample, final_missingness)) %>% 
...
# create matrix
data_matrix <- data %>%
  dplyr::select(ptm_collapse_key, sample, final_intensity_log2) %>%
  pivot_wider(names_from = sample, values_from = final_intensity_log2) %>%
  arrange(ptm_collapse_key) %>%
