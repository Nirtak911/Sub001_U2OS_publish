...
norm_input <- file.path(data_in, "04_0_scale_not_abs_data_log2.csv")
...
movement_fraction_data <- norm_data %>%
  ...
  group_by(ptm_collapse_key, treatment, r_replicate) %>%
...
loss_data <- classified_data %>%
  ...
  dplyr::select(ptm_collapse_key, treatment, r_replicate,
...
movement_combinations_data <- loss_data %>%
  inner_join(
    gain_data,
    by = c("ptm_collapse_key", "treatment", "r_replicate"),
  ...
  left_join(
    movement_fraction_data %>% dplyr::select(ptm_collapse_key, treatment, r_replicate, 
...
combined_data <- movement_combinations_data %>%
  left_join(
    diff_data %>% 
      dplyr::select(ptm_collapse_key, Treatment, comparison,
                    diff, pval, adj_pval, movement, classified_movement) %>%
      dplyr::rename(treatment = Treatment),
    by = c("ptm_collapse_key", "treatment", "movement")
  ) 
View(combined_data)
...
movements_in_all_reps <- combined_data_filtered %>%
  distinct(ptm_collapse_key, treatment, r_replicate, movement) %>% 
  group_by(ptm_collapse_key, treatment, movement) %>%
  summarise(n_reps_with_movement = n(), .groups = "drop") %>%
  group_by(ptm_collapse_key, treatment) %>%
  summarise(
    movements_present_in_all = list(movement[n_reps_with_movement >= 2]),
    .groups = "drop"
  ) %>%
...
combined_data_final <- combined_data_filtered %>%
  inner_join(movements_in_all_reps_expanded %>%
               dplyr::rename(movement = "movements_present_in_all"),
             by = c("ptm_collapse_key", "treatment", "movement"))
...
combined_data_median <- combined_data_final %>%
  group_by(ptm_collapse_key, treatment, movement, classified_movement) %>%
  summarise(
